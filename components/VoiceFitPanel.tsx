  ...
  const tone =
    fit.label === "strong"
      ? {
          label: "Ready",
          bar: "bg-[var(--brand)]",
          chip: "bg-[var(--brand)] text-[color:var(--navy)]",
        }
      ...
      {onImprove && fit.label !== "strong" && (
        <div className="mt-3 pt-3 border-t border-[var(--border-faint)] flex justify-end">
          <button
            ...
            onClick={onImprove}
            disabled={improving}
            className="inline-flex items-center justify-center h-8 px-3 rounded-[var(--r-sm)] bg-[var(--navy)] text-[color:var(--brand)] text-[11px] font-extrabold uppercase tracking-wider hover:opacity-90 disabled:opacity-50 transition"
          >
