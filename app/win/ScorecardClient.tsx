...
export default function ScorecardClient() {
  ...
  return (
    <div className="sc-root">
      {/* dangerouslySetInnerHTML, not {CSS}: as a text child React escapes
          the double quotes in font-family:"Plus Jakarta Sans" to &quot; on the
          server but not on the client, which is a hydration mismatch. React
          then throws away the server HTML and re-renders the whole page. */}
      <style dangerouslySetInnerHTML={{ __html: CSS }} />

      {/* Same lockup as /meet, from the same component. A stranger landing
          cold on a lead magnet should be able to tell whose it is. */}
      <div className="sc-wrap sc-head">
        <a href="/" className="sc-logo" aria-label="Coach Assistant home">
...
.sc-wrap{max-width:620px;margin:0 auto;padding:0 22px}
.sc-head{padding-top:40px;padding-bottom:0}
.sc-logo{display:inline-flex;text-decoration:none;margin-bottom:2.6rem}
.sc-intro,.sc-quiz,.sc-result{padding-bottom:72px}
...
.sc-cta small{display:block;font-weight:500;font-size:.72rem;color:#D6E8D8;margin-top:3px}
.sc-trust{font-size:.8rem;line-height:1.55;color:#6A6A60;margin:18px 0 0;text-align:center}
...
.sc-score small{font-size:1.2rem;color:#6A6A60;font-weight:500}
...
.sc-input::placeholder{color:#6A6A60}
...
.sc-restart button{background:none;border:none;color:#6A6A60;font-family:inherit;font-size:.82rem;text-decoration:underline;cursor:pointer}
